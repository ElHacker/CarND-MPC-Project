...
// The solutuion uses a single vector with all the variables. There is need for
// offseting the indexes to arrive to the correct section for each different
// variable.
size_t x_start = 0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (auto i = 0; i < N-1 ; i++){
    cout << i << ": " << "solution.x[x_start+i]: " << solution.x[x_start+i] <<
      "solution.x[y_start+i]: " << solution.x[y_start+i] << endl;
